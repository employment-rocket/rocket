...
import {
	getIntroduces,
	getIntroduceQuestions,
} from "../../../api/introduce/IntroduceApi.jsx";
...
const IntroduceContainer = ({
	...
	useEffect(() => {
		const fetchQuestions = async () => {
			...
			try {
				const data = await getIntroduceQuestions(
					selectedIntroduce.introduceId,
					memberId
				);
				setQuestions(data);
			} catch (error) {
	...
	return (
		<div className="flex w-full h-full space-x-4">
			<div
				className="w-1/4 border-blue-400 rounded-lg shadow-lg"
				style={{ borderWidth: "3px" }}
			>
				<IntroduceBox
					introduces={introduces}
					onSelectIntroduce={(introduce) =>
						setSelectedIntroduce(introduce)
					}
			...
			<div
				className="w-3/4 border-blue-400 rounded-lg shadow-lg"
				style={{ borderWidth: "3px" }}
			>
