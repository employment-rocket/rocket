...
const IntroduceQuestion = ({
    ...
    useEffect(() => {
        const selected = checkedQuestions?.introduceAnswerList?.some((q) => q.qid === qid);
        ...
        try {
            if (isSelected) {
                ...
                if (confirm) {
                    ...
                    onRemoveCheckedQuestion({ qid, question, category: "INTRODUCE_QA", answerId: newAnswerId });
                }
            } else {
                if (!newAnswerId) {
                    const createdAnswerId = await createAnswer({
                        memberId: 1,
                        category: "INTRODUCE_QA",
                        qid,
