...
const ScriptOverview = ({ checkedQuestions, categories }) => {
	...
	const generatePDF = () => {
		...
		addFontToPdf(pdf);
		pdf.setFont("NotoSansKR-Black", "normal");
		...
		selectedCategories.forEach((value) => {
			...
			questions.forEach((question, index) => {
				...
				if (
					cursorY +
						(questionLines.length + contentLines.length) *
							fontSize *
				...
				cursorY +=
					questionLines.length * fontSize * lineHeightAdjustment;

				pdf.text(contentLines, margin, cursorY);
				cursorY +=
					contentLines.length * fontSize * lineHeightAdjustment + 5;
			});
		});
